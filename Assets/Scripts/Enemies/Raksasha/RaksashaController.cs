...
public class RaksashaController : EnemyController
{
    ...
    internal void ApplySkill(PlayerController target)
    {
        switch (animActive.AnimName)
        {
            case AnimNames.APLASTAR:
    ...
    {
        Quaternion rotationY = Quaternion.LookRotation(rotateTo);
        rotation.eulerAngles = new Vector3(0, rotationY.eulerAngles.y + 10, 0);

        animActive = animInfoList[Random.Range(0, animInfoList.Count)];
        animator.SetTrigger(animActive.Trigger);
    ...
    void Update()
    {
        ...
        if ((isCatchingPlayer || isPatrolling) && !isInAttackRange && !isAttacking)
        {
            Debug.Log("#MOVE can move");
    ...
    public async Task WaitUntilFinishAttack()
    {
        ...
        await new WaitForSeconds(animActive.AnimTime);
        isAttacking = false;
    }
